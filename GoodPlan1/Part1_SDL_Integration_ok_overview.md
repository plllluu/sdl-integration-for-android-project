...
This document outlines the plan to re-integrate SDL-based controller input into the Android input driver. 
The goal is to restore the functionality present in the `src-v1old` version of the code, which allows for robust controller support through the SDL library, and to run this in a headless foreground service for background input processing.
